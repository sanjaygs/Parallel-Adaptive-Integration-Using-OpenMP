...
double integral(double (*f)(double),double a,double b,double tolerance,int n);

//Parallel Algorithm
double integral_par(double (*f)(double),double a,double b,double tolerance,int n);
...
int main()
{
   ...
   answer = integral(func,x_min,x_max,threshold,n);
   ...
   #pragma omp parallel
   {
	   	#pragma omp single
	   	{
	      answer = integral_par(func,x_min,x_max,threshold,n);
	   	}
   }   
   end_time = omp_get_wtime();
...
//Sequential Algorithm
double integral(double (*f)(),double a,double b,double tolerance,int n)
{
	count = count+1; // Increment number of recursive calls by 1
        double result; // Value to return
	double height,h1,h2; // Height of the interval
	double mid; // Center of the interval
	double full_area; // Area of a trapezium formed with parallel edges of length f(a) and f(b)
	double sum_of_halves_area; // Sum of the areas of the two trapeziums: one with parallel edges of length f(a) and f(mid) and the other with parallel edges f(mid) and f(b)
    ...
    else // error is not acceptable
    {
    	...
    	left_area = integral(f,a,mid,tolerance/2,n/2);
    	right_area = integral(f,mid,b,tolerance/2,n/2);
...
//Parallel Algorithm 
double integral_par(double (*f)(),double a,double b,double tolerance,int n)
{
	double result; // Value to return
	double height,h1,h2; // Height of the interval
	double mid; // Center of the interval
...
double x[n+1],x2[n+1],sum=0,x1[n+1],s2=0,s1=0;
    ...
    else // error is not acceptable
    {
        ...
        {
      	   left_area = integral_par(f,a,mid,tolerance/2,n/2);
        }

        #pragma omp task shared(right_area)
        {
    	   right_area = integral_par(f,mid,b,tolerance/2,n/2);
        }
