...
#pragma omp critical (stack)
      ...
      h = b - a;
      mid = (a+b)/2;
      one_trapezoid_area = (h/6) * (f(a)+f(b)+(4*f((a+b)/2)));
      two_trapezoid_area = ( (h/12) * (f(a)+f(mid)+(4*f((a+mid)/2)))) + ((h/12) * (f(b)+f(mid)+(4*f((b+mid)/2))));
...
main()
{
	...
	scanf("%lf",&y);
	scanf("%lf",&x); 
	printf("Definite integral is %lf\n",integralSeq(f,x,y,0.001));
	printf("Definite integral is %lf\n",integral(f,x,y,0.001));
}
